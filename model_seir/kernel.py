...
def main(configuration, save_data: bool, analyze_data: bool, erase_prev_data: bool, save_percentage: float, name="", save_pictures=False, files_sufix=''):

    ## Leer configuracion y abrir ficheros de guardado de datos
    country = configuration["country"]
    if save_data:
        files = open_save_files(country, erase_prev=erase_prev_data, sufix=files_sufix)
    ...
    for execution in range(configuration["simulation"]["n_executions"]):
        ...
        # params[0,0] = 0.2
        # params[1,0] = 60.00299680788351e-06
        states = prepare_states(params, configuration["total_population"])

        ## Simular estados
        evolve_gpu(params, fixed_params, states, deaths_list, log_diff, configuration)
        ...
        if execution==0:
            recovered_array = plot_states(best_params, fixed_params, deaths_list, configuration, name=name)
