...
def update_geo_frame(frame):
    ...
    ppp_gdp = pd.read_csv("real_data/ppp_world.csv")
    ...
    for c in frame['name']:
        ...
        frame.loc[lambda w: w['name']==c, "ppp_per_capita"] = float(ppp_gdp.loc[lambda w: w['name']==c, "ppp_per_capita"])
        ...
        for v,f in files.items():
            ...
            v_array_sorted = v_array.copy()
            v_array_sorted.sort()

            if len(v_array) > 0:
                frame.loc[lambda w: w['name']==c, v] = analysis.median(v_array_sorted)
                frame.loc[lambda w: w['name']==c, v+"_err"] = v_array.std()

        config = read_configuration(c, prefix="used/", sufix='_final', v2=True)
