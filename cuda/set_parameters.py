...
def set_lambda(params, *, size=N_SIMULATIONS):
    #'lambda'
    params[1] = cprand.random(size, dtype=cp.float64) * (0.3-0.01) + 0.01

def set_IFR(params, *, size=N_SIMULATIONS):
    #'IFR'
    params[2] = cprand.random(size, dtype=cp.float64) * (0.025-0.005)*2 + 0.005

def set_what(params, *, size=N_SIMULATIONS): # xi
    #'what'
    params[3] = cprand.random(size, dtype=cp.float64) * (1/6-1/28) + 1/28 #! * 1/7 y 1/21

def set_initial_i(params, *, size=N_SIMULATIONS):
    #'initial_i'
    params[4] = cprand.random(size, dtype=cp.float64) * 0.5E-8
    
def set_mu(params, *, size=N_SIMULATIONS):
    #'mu'
    params[5] = cprand.random(size, dtype=cp.float64) * (0.4-0.1) + 0.1 # 1/4.2
    
def set_eta(params, *, size=N_SIMULATIONS):
    #'eta'
    params[6] = cprand.random(size, dtype=cp.float64) * (0.4-0.1) + 0.1 # 1/5.2
...
def set_params(params, *, size=N_SIMULATIONS):
    ...
    set_mu(params, size=size)
    set_eta(params, size=size)
