...
def prepare_states(params, total_population):
    ...
    state = cp.zeros((4,params.shape[1]), dtype=cp.float64)
    state[0] = 1-params[param_to_index['initial_i']]
    state[2] = params[param_to_index['initial_i']]
    return state


# Volver a ejecutar los mejores para plotearlos
def plot_states(params, fixed_params, deaths_list, configuration, name=""):
    ...
    time = cp.zeros(params.shape[1], dtype=cp.int32)
    ...
    # while __time_ref<NEW_MAX_DAYS:
        ...
        evolve(params, fixed_params, states)
        
        deaths[_time * (_time>=0) * (NEW_MAX_DAYS>_time),_range] += states[3,_range]*TOTAL_POPULATION * (_time<NEW_MAX_DAYS) * (_time>=0)
    ...
    with plt.style.context('science'):
        fig_, ax_ = plt.subplots(figsize=(8/1.3,5/1.3))
        _visibility = 0.1

        time_list =  np.arange(NEW_MAX_DAYS)

        ax_.plot(time_list[:MAX_DAYS], deaths_list.get()[0:MAX_DAYS], '-', color='red', label='Datos usados')
        ...
        ax_.plot(time_list, _median, ':', color='purple', label='Ajuste')
        ...
        # ax_p_active.set_ylabel('Movilidad')
        ...
        ax_.set_ylabel('Muertes diarias')
        ...
        ticks = [i for i in range(0, NEW_MAX_DAYS, 20)]
